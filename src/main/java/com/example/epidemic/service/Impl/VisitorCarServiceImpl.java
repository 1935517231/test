...
import com.example.epidemic.dao.VisitorCarDao;
...
@Service
public class VisitorCarServiceImpl implements VisitorCarService {
    @Autowired
    private VisitorCarDao carDao;

    @Override
    public ResultBean add(VisitorCar visitorCar) {
        if (carDao.add(visitorCar) > 0) {
    ...
    @Override
    public List<VisitorCar> selectAll() {
    ...
    public PageBean queryPageWhere(String name, int page, int limit) {
        ...
        //3.获取分页结果
        PageInfo<VisitorCar> pageInfo = new PageInfo<>(carDao.selectWhere(name));
        //4.返回分页的数据
    ...
    public ResultBean addBatch(MultipartFile file) throws Exception {
        if (!file.isEmpty()) {
            List<VisitorCarExcel> list = EasyExcel.read(file.getInputStream(), VisitorCarExcel.class, new ReadListener() {
                @Override
